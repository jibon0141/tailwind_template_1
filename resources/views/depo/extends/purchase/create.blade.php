    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function medicineSearch(e, row) {
                ...
                const oldBox = row.querySelector('.medicine-suggestion-box');
                if (oldBox) oldBox.remove();

                if (!query) return;

                fetch(`{{ route('depo.purchase.searchMedicine') }}?q=${query}`)
                    .then(r => r.json())
                    .then(d => {
                        if (!d.length) return;

                        const box = document.createElement('div');
                        box.className = 'medicine-suggestion-box absolute bg-white border z-50 w-full';
                        ...
                        d.forEach(m => {
                            ...
                            div.onclick = () => {
                                ...
                                box.remove(); // close dropdown after select
                                calculate();
                            };
