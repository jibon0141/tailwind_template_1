        ...
        <div class="min-h-screen bg-gray-100 p-4">
            <div class="mx-auto">
                ...
                <div class="flex justify-between items-center mb-4">
                    ...
                    <a href="{{ route('mpo.sale.index') }}" class="px-3 py-1.5 bg-indigo-600 text-white rounded text-sm">
                        📋 Sale List
                    </a>
                </div>
                ...
                <div class="grid grid-cols-12 gap-3 mb-4">
                    <div class="col-span-12 md:col-span-6">
                        ...
                        <select name="shop_id" id="shopSelect" class="w-full border rounded px-2 py-1">
                    ...
                    <div class="col-span-12 md:col-span-6">
                        ...
                        <input type="date" name="sale_date" value="{{ date('Y-m-d') }}" class="w-full border rounded px-2 py-1">
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-2 mt-4">
                    <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded">Save Sale</button>
                </div>
            </div>
        </div>
    </form>
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            fetch("{{ route('mpo.sale.getSaleData') }}")
                .then(r => r.json())
                .then(d => {
                    console.log(d)
                    d.chemistShops.forEach(s => {
                        shopSelect.innerHTML += `<option value="${s.id}" >${s.shop_name}</option>`;
                    });
            ...
            function calculate() {
                ...
                let finalTotal = total - discount + vatAmount - advance;
                document.getElementById('final_total').value = finalTotal.toFixed(2);
            }

            function medicineSearch(e, row) {
                fetch(`{{ route('mpo.sale.searchMedicine') }}?q=${e.target.value}`)
                    .then(r => r.json())
                    .then(d => {
                        if (!d.length) return;
