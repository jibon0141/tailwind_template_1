...
@section('content')
    <form method="POST" action="{{ route('admin.temp-distribute.create') }}">
        @csrf
        ...
        <div class="min-h-screen bg-gray-100 p-4">
            <div class="mx-auto">
                ...
                <div class="flex justify-between items-center mb-4">
                    ...
                    <a href="{{ route('admin.temp-distribute.index') }}" class="px-3 py-1.5 bg-indigo-600 text-white rounded text-sm">
                        📋 Distribute List
                    </a>
                </div>
                    ...
                    <div class="col-span-12 lg:col-span-9 bg-white rounded shadow p-3">
                        ...
                        <table class="w-full border text-sm">
                            ...
                            @foreach(old('items', [0 => []]) as $index => $item)
                                ...
                                {{-- Row-level error display for multiple errors --}}
                                @if(
                                    ...
                                    <tr>
                                        <td colspan="7" class="bg-red-50 px-3 py-2">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function medicineSearch(e, row) {
                ...
                // collect already selected medicine IDs
                let selectedIds = Array.from(document.querySelectorAll('.medicine_id'))
                    .map(i => i.value)
                    .filter(Boolean);

                // allow same row reselect (edit safety)
                const currentId = row.querySelector('.medicine_id').value;
                if (currentId) {
                ...
                fetch(`{{ route('admin.distribute.searchMedicine') }}?q=${encodeURIComponent(query)}`)
                    ...
                    .then(data => {
                        ...
                        const filtered = data.filter(m => !selectedIds.includes(m.id.toString()));
                        if (!filtered.length) return;

                        const box = document.createElement('div');
                        box.className =
                            'medicine-suggestion-box absolute bg-white border z-50 w-full shadow max-h-60 overflow-auto';
                        ...
                        filtered.forEach(m => {
                            ...
                            div.onclick = () => {
                                ...
                                // Fill the new Stock column
                                row.querySelector('.stock').value = m.current_stock;

                                // Optional: restrict quantity to stock
                                const qtyInput = row.querySelector('.quantity');
                                qtyInput.max = m.current_stock;
                                if (parseFloat(qtyInput.value) > m.current_stock) {
                                    qtyInput.value = m.current_stock;
                                }
