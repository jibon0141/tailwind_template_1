...
@section('content')
    <div class="page-wrapper bg-gray-100 min-h-screen p-6">
        ...
        <div class="bg-white p-6 rounded shadow mb-6 flex flex-col md:flex-row items-center justify-between">
            <h3 class="text-2xl font-semibold text-gray-800">Supplier Payment List</h3>

            <a href="{{ route('admin.supplier-due-payment.create') }}"
               class="mt-3 md:mt-0 inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2 rounded shadow transition">
                <i class="fa fa-plus"></i> Create Supplier Payment
            </a>
        </div>
        ...
        <div class="bg-white p-4 rounded shadow mb-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 items-end">
                ...
                <div class="flex flex-col md:flex-row gap-2">
                    <button id="filterBtn"
                            class="h-10 w-full md:w-full bg-teal-500 text-white rounded hover:bg-teal-600 transition">
                        Filter
                    </button>
                    <button id="resetBtn"
                            class="h-10 w-full md:w-full bg-gray-500 text-white rounded hover:bg-gray-600 transition">
                        Reset
                    </button>
                </div>
        ...
        <div class="bg-white p-6 rounded shadow overflow-x-auto">
            <table id="supplierPaymentTable" class="w-full table-auto text-sm">
...
@section('scripts')
    <script>
        $(document).ready(function () {
            var table = $('#supplierPaymentTable').DataTable({
                processing: true,
                serverSide: true,
                scrollX:true,
                autoWidth: false,
                ajax: {
                    url: "{{ route('admin.supplier-due-payment.index') }}",
                    data: function(d) {
