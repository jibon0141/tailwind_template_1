...
@section('content')
    ...
    <div class="min-h-screen bg-gray-100 p-2 md:p-6">
        ...
        <div class="mb-4 flex justify-end print:hidden">
            <a href="{{ route('report.depo.profit') }}"
        ...
        <div id="invoice" class="max-w-5xl mx-auto bg-white shadow rounded-lg p-4 md:p-6">
            ...
            <div class="border-b pb-4 mb-6">
                ...
                <div class="flex flex-row justify-between items-center mb-4">
                    <div>
                        ...
                        <p class="text-sm text-gray-600">{{ $sale->chemistHouse->address ?? 'N/A' }}</p>
                        <p class="text-sm text-gray-600">{{ $sale->chemistHouse->contact ?? 'N/A' }}</p>
                        <p class="text-sm text-gray-600 font-bold">
                            Submitted By : <span class="text-base">{{ $sale->depo->person_name }}</span>
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-600 mb-1">Delivery Date : {{
                            $sale->delivery_date ? \Carbon\Carbon::parse($sale->delivery_date)->format('d M Y') : 'N/A'
                        }}</p>
                ...
                <div class="flex flex-col md:flex-row md:justify-evenly gap-2 text-center md:text-left">
                    ...
                    <p class="text-sm font-semibold">
                        Order Date : {{ $sale->sale_date ? \Carbon\Carbon::parse($sale->sale_date)->format('d M Y') : 'N/A' }}
                    </p>
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full border border-gray-200 text-xs md:text-sm">
                    ...
                    <tr>
                        ...
                        <th class="border px-3 py-2 text-right">Purchase Price</th>
                        <th class="border px-3 py-2 text-right">Sale Price</th>
                        <th class="border px-3 py-2 text-right">Profit</th>
                        ...
                        <th class="border px-3 py-2 text-right">Sub Profit</th>
                    ...
                    @forelse($sale->items ?? [] as $index => $item)
                        <tr>
                            ...
                            <td class="border px-3 py-2 text-right">{{ $item->medicine->medicine_name ?? 'N/A' }}</td>
                            <td class="border px-3 py-2 text-right">{{ $item->medicine->purchase_price ?? 0,2 }}</td>
                            <td class="border px-3 py-2 text-right">{{ number_format($item->unit_cost ?? 0, 2) }}</td>
                            <td class="border px-3 py-2 text-right">{{ number_format($item->unit_cost - $item->medicine->purchase_price, 2) }}</td>
                            <td class="border px-3 py-2 text-right">{{ number_format($item->medicine_discount ?? 0, 2) }}</td>
                            <td class="border px-3 py-2 text-right">{{ $item->quantity ?? 0 }}</td>
                            <td class="border px-3 py-2 text-right">{{ $item->free_quantity ?? 0 }}</td>
                            <td class="border px-3 py-2 text-right">
                                @php
                                    $purchasePrice = $item->medicine->purchase_price ?? 0;
                                    $salePrice = $item->unit_cost ?? 0;
                                    $qty = $item->quantity ?? 0;
                                    $discount = $item->medicine_discount ?? 0;
                            ...
                            <td class="border px-3 py-2 text-right">{{ number_format($item->sub_total ?? 0, 2) }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="8" class="border px-3 py-4 text-center text-gray-500">No items found</td>
                        </tr>
                    @endforelse
                    </tbody>
                </table>
            ...
            <!-- Footer -->
            <div class="mt-6 flex justify-between items-center gap-2 text-sm text-gray-500 print:hidden">
                <p>Generated on {{ now('Asia/Dhaka')->format('d M Y, h:i A') }}</p>

                <div class="">
                    <a href="{{ route('report.depo.print.profit', $sale->id) }}">
                        <button class="px-4 py-2 bg-teal-700 text-white rounded hover:bg-teal-800 mr-2">Print View</button>
                    </a>
