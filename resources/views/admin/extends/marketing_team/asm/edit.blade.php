...
@section('content')
    <div class="min-h-screen bg-gray-100 p-6">
        <div class="max-w-7xl mx-auto">
            ...
            <form action="{{ route('asm.update', $employee->id) }}" method="POST" class="bg-white p-6 rounded shadow">
                @csrf
                @method('PUT')
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    ...
                    <div class="">
                        ...
                        <input type="text" value="ASM" class="w-full border p-2 rounded bg-gray-200" disabled>
                    ...
                    <div>
                        <label class="block text-sm font-medium mb-1">Parent Director</label>
                        <select name="parent_id" class="border p-2 rounded w-full">
                            <option value="">Select Director</option>
                            @foreach($sms as $sm)
                                <option value="{{ $sm->id }}" {{ $employee->parent_id == $sm->id ? 'selected' : '' }}>
                                    {{ $sm->full_name }}  | {{ $sm->employee_code ?? '' }}
                                </option>
                            @endforeach
                        </select>
