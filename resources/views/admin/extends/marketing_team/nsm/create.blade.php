...
@section('content')
    <div class="min-h-screen bg-gray-100 p-6">
        <div class="max-w-7xl mx-auto">
            ...
            <form action="{{ route('nsm.create') }}" method="POST" class="bg-white p-6 rounded shadow">
                @csrf
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    ...
                    <div class="">
                        ...
                        <input type="text" value="NSM" class="w-full border p-2 rounded bg-gray-200" disabled>
                        <input type="hidden" name="employee_type" value="nsm">
                    </div>

                    {{-- Parent Employee (Directors only) --}}
                    <div>
                        <label for="parent_id" class=" mb-1 block text-sm font-medium text-gray-700">Parent Employee <span class="text-red-500">*</span></label>
                        <select name="parent_id" class="border p-2 rounded w-full">
                            <option value="">Select Director</option>
                            @foreach($directors as $director)
                                <option value="{{ $director->id }}" {{ old('parent_id') == $director->id ? 'selected' : '' }}>
                                    {{ $director->full_name }}  | {{ $director->employee_code ?? '' }}
                                </option>
                            @endforeach
                        </select>
