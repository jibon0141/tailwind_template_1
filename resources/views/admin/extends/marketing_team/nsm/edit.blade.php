...
@section('content')
    <div class="min-h-screen bg-gray-100 p-6">
        <div class="max-w-7xl mx-auto">
            ...
            <form action="{{ route('nsm.update', $employee->id) }}" method="POST" class="bg-white p-6 rounded shadow">
                @csrf
                @method('PUT')
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium mb-1">Parent Director <span class="text-red-500">*</span></label>
                        <select name="parent_id" class="border p-2 rounded w-full">
                            <option value="">Select Director</option>
                            @foreach($directors as $director)
                                <option value="{{ $director->id }}" {{ $employee->parent_id == $director->id ? 'selected' : '' }}>
