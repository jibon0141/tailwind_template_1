...
@section('content')
    <div class="min-h-screen bg-gray-100 p-6">
        <div class="max-w-7xl mx-auto">
            ...
            <form action="{{ route('mpo.update', $employee->id) }}" method="POST" class="bg-white p-6 rounded shadow">
                @csrf
                @method('PUT')
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    ...
                    <div class="">
                        ...
                        <input type="text" value="MPO" class="w-full border p-2 rounded bg-gray-200" disabled>
                        <input type="hidden" name="employee_type" value="mpo">
                    </div>


                    {{-- Parent Employee (Sm only) --}}
                    <div>
                        ...
                        <select name="parent_id" class="border p-2 rounded w-full">
                            ...
                            @foreach($asms as $asm)
                                <option value="{{ $asm->id }}" {{ $employee->parent_id == $asm->id ? 'selected' : '' }}>
                                    {{ $asm->full_name }}  | {{ $asm->employee_code ?? '' }}
                                </option>
                            @endforeach
                        </select>
