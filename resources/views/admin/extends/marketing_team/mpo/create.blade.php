    ...
    <div class="min-h-screen bg-gray-100 p-6">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="mb-6 mt-2 pt-6 flex items-center justify-between">
                <h3 class="text-2xl font-bold text-gray-800">Create New MPO</h3>
            ...
            <form action="{{ route('mpo.create') }}" method="POST" class="bg-white p-6 rounded shadow">
                @csrf
                ...
                <h4 class="font-semibold mt-8 mb-4 text-lg">Assign Parent ASM</h4>
                <div class="grid md:grid-cols-2 gap-4">

                    {{-- Employee Type (readonly) --}}
                    <div>
                        <label class="block text-sm font-medium mb-1">Employee Type</label>
                        <input type="text" value="MPO" class="w-full border p-2 rounded" readonly>
                        <input type="hidden" name="employee_type" value="mpo">
                    </div>

                    <div>
                        <label class="block text-sm font-medium mb-1">Parent ASM <span class="text-red-500">*</span></label>
                        <select name="parent_id" class="w-full border p-2 rounded">
                            <option value="">Select ASM</option>
                            @foreach($asms as $asm)
                                <option value="{{ $asm->id }}" {{ old('parent_id') == $asm->id ? 'selected' : '' }}>
                                    {{ $asm->full_name }} {{ $asm->last_name }} | {{ $asm->employee_code ?? '' }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mt-6 text-right">
                    <button type="submit" class="bg-teal-500 hover:bg-teal-600 text-white px-6 py-2 rounded">Create MPO</button>
                </div>
