    ...
    <div class="min-h-screen bg-gray-100 p-6">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="mb-6 mt-2 pt-6 flex items-center justify-between">
                <h3 class="text-2xl font-bold text-gray-800">Update SM</h3>
                <a href="{{ route('sm.index') }}" class="inline-flex items-center gap-2 rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow hover:bg-blue-700 transition">
                    <i class="fa fa-reply"></i> SM List
                </a>
            </div>

            <form action="{{ route('sm.update', $employee->id) }}" method="POST" class="bg-white p-6 rounded shadow">
                @csrf
                @method('PUT')
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email', $employee->email) }}" class="w-full border p-2 rounded">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="address" class="w-full border p-2 rounded">{{ old('address', $employee->address) }}</textarea>
                        @error('address') <p class="text-red-500 text-sm">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label class="block text-sm font-medium mb-1">Password</label>
                        <input type="password" name="password" class="w-full border p-2 rounded" placeholder="Leave blank if not changing">
                        @error('password') <p class="text-red-500 text-sm">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label class="block text-sm font-medium mb-1">Confirm Password</label>
                        <input type="password" name="password_confirmation" class="w-full border p-2 rounded" placeholder="Confirm password">
                    </div>
                </div>
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" class="w-full border p-2 rounded bg-gray-100" value="SM" disabled>
                        <input type="hidden" name="employee_type" value="sm">
                    </div>

                    <div>
                        <label class="block text-sm font-medium mb-1">Parent RSM <span class="text-red-500">*</span></label>
                        <select name="parent_id" class="w-full border p-2 rounded">
                            <option value="">Select RSM</option>
                            @foreach($rsms as $rsm)
                                <option value="{{ $rsm->id }}" {{ old('parent_id', $employee->parent_id) == $rsm->id ? 'selected' : '' }}>
                                    {{ $rsm->full_name }} | {{ $rsm->employee_code ?? '' }}
                                </option>
                            @endforeach
                        </select>
