    ...
    <div class="min-h-screen bg-gray-100 p-6">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="mb-6 mt-2 pt-6 flex items-center justify-between">
                <h3 class="text-2xl font-bold text-gray-800">Create New RSM</h3>

                <a href="{{ route('rsm.index') }}"
            ...
            <form action="{{ route('rsm.create') }}" method="POST" class="bg-white p-6 rounded shadow">
                @csrf
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="full_name" value="{{ old('full_name') }}" placeholder="Full Name" class="w-full border p-2 rounded">
                        @error('full_name') <p class="text-red-500 text-sm">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label for="email" class="block text-sm font-medium text-gray-700">Email <span class="text-red-500">*</span></label>
                        <input type="email" name="email" value="{{ old('email') }}" placeholder="Email" class="w-full border p-2 rounded">
                        @error('email') <p class="text-red-500 text-sm">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label for="phone" class="block text-sm font-medium text-gray-700">Phone <span class="text-red-500">*</span></label>
                        <input type="text" name="phone" value="{{ old('phone') }}" placeholder="Phone" class="w-full border p-2 rounded">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="address" placeholder="Address" class="w-full border p-2 rounded">{{ old('address') }}</textarea>
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" value="RSM" class="w-full border p-2 rounded bg-gray-200" disabled>
                        <input type="hidden" name="employee_type" value="rsm">
                    </div>

                    <div>
                        <label class="block text-sm font-medium mb-1">Parent Employee (NSM) <span class="text-red-500">*</span></label>
                        <select name="parent_id" class="w-full border p-2 rounded">
                            <option value="">Select NSM</option>
                            @foreach($nsms as $nsm)
                                <option value="{{ $nsm->id }}" {{ old('parent_id') == $nsm->id ? 'selected' : '' }}>
                                    {{ $nsm->full_name }} | {{ $nsm->employee_code ?? '' }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mt-6 text-right">
                    <button type="submit" class="bg-teal-500 hover:bg-teal-600 text-white px-6 py-2 rounded">Save RSM</button>
                </div>
