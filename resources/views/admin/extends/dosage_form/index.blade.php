...
@section("content")
    ...
    <div class="mt-8 bg-white p-6 rounded shadow overflow-x-auto">
        <table id="dosageTable" class="w-full">
            <thead>
            <tr>
                <th>#</th>
                <th>Dosage Name</th>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function () {
            $('#dosageTable').DataTable({
                ...
                scrollX:true,
                autoWidth: false,
                ajax: "{{ route('dosage.index') }}",
                columns: [
                    { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false },
                    { data: 'dosage_name', name: 'dosage_name' },
                    { data: 'dosage_description', name: 'dosage_description' },
    ...
    //     Delete Ajax
        ...
        function deleteItem(id) {
            Swal.fire({
                title: 'Are you sure?',
                text: 'This dosage will be permanently deleted!',
                icon: 'warning',
                        ...
                        .then(data => {
                            if (data.success) {
                                Swal.fire({
                                    icon: 'success',
                                    title: 'Deleted!',
                                    text: data.message ?? 'Dosage deleted successfully',
                                    timer: 1500,
                                    showConfirmButton: false
                                });

                                $('#dosageTable').DataTable().ajax.reload(null, false);
                            } else {
                                Swal.fire({
                                    icon: 'error',
                                    title: 'Failed!',
