...
class ChemistHouseLedgerController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('depo.extends.report.chemist_house_ledger_report.index');
    }
    ...
    public function getChemistHouses(Request $request)
    {
        ...
        $search = $request->q;
        $query = ChemistHouse::where('depo_id', $depoId);

        if ($search) {
            $query->where('shop_name', 'like', "%$search%");
        }

        $suppliers = $query->orderBy('shop_name')->limit(50)->get();
