...
namespace App\Http\Controllers\Backend\HeadOfficePurchase;
...
class HeadOfficePurchaseController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            return DataTables::of($purchases)
                ...
                ->addColumn('payment_status', function ($row) {
                    return match ($row->payment_status) {
                        ...
                        default => '<span class="px-2 py-1 bg-gray-300 ttext-black rounded-lg text-xs font-semibold">N/A</span>',
                    };
                })
                ->addColumn('action', function ($row) {
                    return '<a href="' . route('admin.purchase.show', $row->id) . '"
            ...
            return view('admin.extends.purchase.index');
    }
    ...
    public function create(Request $request)
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            try {
                ...
                SupplierLedger::create([
                    ...
                    'credit'        => $givenAmount,
                    'balance'       => 0,
                    'voucher_route' => 'admin.purchase.show',
               ...
               // CashFlow Section
                ...
                return redirect('/admin/purchase/show/'.$purchase->id)->with('success', 'Purchase created successfully.');
        ...
        return view('admin.extends.purchase.create');
    }
    ...
    public function show($id)
    {
        try{
            ...
            return view('admin.extends.purchase.show', compact('purchase','mainCompany'));
        }
        catch (\Exception $e) {
            Log::error($e->getMessage());
    ...
    public function print($id){
        ...
        try{
            ...
            return view('admin.print.purchase_voucher_print', compact('purchase','mainCompany'));
        }
        catch (\Exception $e) {
            Log::error($e->getMessage());
            return redirect()->back()->with('error', 'Purchase Show Failed!');
    ...
    public function subAdminPurchase(Request $request){

        if ($request->ajax()) {

            $purchases = Purchase::with('supplier', 'account','user')
                ->where('user_id','!=',1)
                ->where('depo_id', 0)
                ->orderBy('id', 'desc');
            ...
            return DataTables::of($purchases)
                ...
                ->addColumn('purchased_by', fn($row) =>$row->user->name
                )
        ...
        return view('admin.extends.purchase.sub_admin_purchase');
...
//            'suppliers' => Supplier::select('id','supplier_name','balance')->get(),
            'accounts'  => Account::select('id','account_name','balance','is_default')->where('user_id',Auth::id())
                ->where('depo_id',0)
                ->get(),
        ]);
    }
