<?php

namespace App\Http\Controllers\Backend\Brand;

use App\Http\Controllers\Controller;
use App\Models\Brand;
use App\Models\Medicine;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use Yajra\DataTables\Facades\DataTables;

class BrandController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            try {
                $brands = Brand::get();

                return DataTables::of($brands)
                    ->addIndexColumn()
                    ->addColumn('brand_name', fn ($row) => $row->brand_name ?? 'N/A')
                    ->addColumn('brand_description', fn ($row) => $row->brand_description ?? 'N/A')
                    ->addColumn('status', function ($row) {
                        return $row->status
                            ? '<span class="inline-block px-4 py-1 text-xs font-semibold text-green-800 bg-green-200 rounded-lg">Active</span>'
                            : '<span class="inline-block px-4 py-1 text-xs font-semibold text-red-800 bg-red-200 rounded-lg">Inactive</span>';
                    })
                    ->addColumn('action', function ($row) {
                        $editUrl = route('brand.edit', $row->id);

                        $btn  = '<div class="flex gap-2">';
                        $btn .= '<a href="'.$editUrl.'" class="inline-flex items-center px-2 py-1 bg-blue-500 hover:bg-blue-600 text-white text-xs rounded" title="Edit"><i class="fa fa-edit"></i></a>';
                        $btn .= '<button onclick="deleteItem('.$row->id.')" class="inline-flex items-center px-2 py-1 bg-red-500 hover:bg-red-600 text-white text-xs rounded" title="Delete"><i class="fa fa-trash"></i></button>';
                        $btn .= '</div>';

                        return $btn;
                    })
                    ->rawColumns(['status', 'action'])
                    ->make(true);

            } catch (\Exception $e) {
                Log::error($e->getMessage());
                return response()->json(['error' => 'Something went wrong'], 500);
            }
        }

        return view('admin.extends.brand.index');
    }

    public function create(Request $request)
    {
        if ($request->isMethod('POST')) {
            $request->validate([
                'brand_name'        => 'required|unique:brands,brand_name',
                'brand_description'=> 'nullable',
                'status'            => 'required',
            ]);



            try {
                Brand::insert([
                    'brand_name'        => $request->brand_name,
                    'brand_description' => $request->brand_description,
                    'status'            => $request->status,
                    'created_at'        => date('Y-m-d H:i:s'),
                ]);

                Log::info('Brand created successfully.');
                return redirect()->back()->with('success', 'Brand created successfully.');

            } catch (\Exception $e) {
                Log::error($e->getMessage());
                return redirect()->back()->with('error', 'Brand create unsuccessful.');
            }
        }

        return view('admin.extends.brand.create');
    }

    public function show($id)
    {
        //
    }

    public function edit($id)
    {
        try {
            $data = Brand::where('id', $id)->first();
            return view('admin.extends.brand.edit', compact('data'));

        } catch (\Exception $e) {
            Log::error($e->getMessage());
            return redirect()->back()->with('error', 'Something went wrong.');
        }
    }

    public function update(Request $request, $id)
    {

        $request->validate([
            'brand_name'        => 'required|unique:brands,brand_name,' . $id,
            'brand_description'=> 'nullable',
            'status'            => 'required|boolean',
        ]);

        try {
            Brand::where('id', $id)->update([
                'brand_name'        => $request->brand_name,
                'brand_description' => $request->brand_description,
                'status'            => $request->status,
                'updated_at'        => date('Y-m-d H:i:s'),
            ]);

            Log::info("Brand updated successfully for id: {$id}");
            return redirect()->back()->with('success', 'Brand updated successfully.');

        } catch (\Exception $e) {
            Log::error($e->getMessage());
            return redirect()->back()->with('error', 'Brand Update Unsuccessful.');
        }
    }

    public function destroy($id)
    {
        try {
            $brand = Brand::find($id);

            if (!$brand) {
                Log::info("Brand Not Found For Delete", ['id' => $id]);
                return response()->json([
                    'success' => false,
                    'message' => 'Brand Not Found.'
                ], 404);
            }

            $hasMedicine = Medicine::where('brand_id', $id)->exists();

            if ($hasMedicine) {
                Log::info("Brand delete blocked due to medicines", ['brand_id' => $id]);
                return response()->json([
                    'success' => false,
                    'message' => 'Cannot delete brand. Medicines exist under this brand.'
                ], 422);
            }

            $brand->delete();

            Log::info("Brand deleted successfully for id: {$id}");
            return response()->json([
                'success' => true,
                'message' => 'Brand deleted successfully.'
            ], 200);

        } catch (\Exception $e) {
            Log::error($e->getMessage());
            return response()->json([
                'success' => false,
                'message' => 'Something went wrong.'
            ], 500);
        }
    }
}
